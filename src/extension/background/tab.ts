...
async function inTab<A extends unknown[], R>(tabId: number, func: (...args: A) => R | Promise<R>, args: A, timeoutMs = SCRIPT_TIMEOUT_MS): Promise<R> {
  ...
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => reject(new SessionEndedError('the Maccabi Online tab did not respond for ' + timeoutMs / 1000 + ' s')), timeoutMs);
  });
  try {
    const run = chrome.scripting.executeScript({ target: { tabId }, world: 'MAIN', func, args }).then((res) => {
      if (!res || !res.length) throw new SessionEndedError('could not run in the Maccabi Online tab');
      return res[0].result as R;
    }, (e: unknown) => {
      // No tab, a tab on another site, or an error page.
      throw new SessionEndedError('the Maccabi Online tab is not available (' + (e instanceof Error ? e.message : String(e)) + ')');
    });
...
export async function isVisible(tabId: number): Promise<boolean> {
  ...
  try {
    ...
    throw new SessionEndedError('the Maccabi Online tab was closed');
  }
...
export async function navigate(tabId: number, url: string, expectPath: RegExp): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    chrome.tabs.update(tabId, { url }).catch((e) => {
      clearTimeout(timer);
      chrome.tabs.onUpdated.removeListener(listener);
      reject(new SessionEndedError('the Maccabi Online tab was closed (' + e.message + ')'));
    });
  });
